...
const Zone = () => {
  ...
  const [chooseZone, setChooseZone] = useState(0);

  const handleSelectionChange = (newValue) => {
    setSelectedZone(newValue);
  };

  const getChoosedZone = (newValue) => {
    console.log("-getChoosedZone", newValue);
    setChooseZone(newValue);
  };
      ...
      <div className="w3-row  w3-margin-bottom">
        <div className="w3-col l3 m3 s6 ">
          <ZoneDropDown
            ...
            choosedZone={getChoosedZone}
          />
        </div>
        <div className="w3-col l3 m3 s6 w3-right ">
          <span className=" w3-small w3-text-gray w3-right">
            {" "}
            * All figures are in Lacs (INR){" "}
          </span>
        </div>
      </div>
